...
before(async () => {
  // Each test file runs in its own process; never load production credentials.
...
beforeEach(async () => {
  const { createApi } = await import("../server/api");
  const api = createApi(new DB());
  POST = (req, info) => api.POST(req, { ip: "192.0.2.1", ...info });
  GET = api.GET;
...
function request(
  ...
  url = "https://www.iamthefold.com/api",
) {
...
function sampleDB(data: Record<string, unknown> | null) {
  return new DB({
    ...
    eval: async () => {
      throw new Error("Sampling must not write");
    },
  });
}
...
test("invalid submissions are rejected before token validation or database writes", async () => {
  for (const fold of [
    ...
    const response = await POST(
      request({ fold, token: "invalid", proof: "1" }),
    );
    assert.equal(response.status, 400);
    assert.equal((await response.json()).message, "Invalid fold");
  }
  for (const body of [null, [], "bad", 123])
...
test("foreign, absent, null and wrong-scheme origins are rejected", async () => {
  for (const origin of [
    ...
    "https://www.iamthefold.com.evil.example",
  ]) {
  ...
  assert.equal(
    (await POST(request(submission(), { "sec-fetch-site": "cross-site" })))
      .status,
    403,
  );
  assert.equal(fakeStore.eval.mock.callCount(), 0);
...
test("same-origin JSON works behind Fly's internal bind address and on localhost", async () => {
  assert.equal(
    (await POST(request(submission(), {}, "https://0.0.0.0:3000/api"))).status,
    200,
  );
  const { createApi } = await import("../server/api");
  assert.equal(
    (
      await createApi(new DB()).POST(
        request(
          submission(),
...
test("text/plain and missing content types are rejected; JSON charset is allowed", async () => {
  for (const type of ["text/plain", "", "application/x-www-form-urlencoded"]) {
    assert.equal(
      (await POST(request(submission(), { "content-type": type }))).status,
      415,
    );
  }
  assert.equal(fakeStore.eval.mock.callCount(), 0);
...
test("malformed JSON and oversized bodies are rejected", async () => {
  ...
  const bad = new Request(malformed.url, {
    method: "POST",
  ...
  assert.equal(
    (await POST(request({ padding: "x".repeat(4096) }))).status,
    413,
  );
  assert.equal(
    (await POST(request({}, { "content-length": "5000" }))).status,
    413,
  );
  assert.equal(
    (
      await POST(
        request({ padding: "é".repeat(2100) }, { "content-length": "1" }),
      )
    ).status,
    413,
  );
...
test("bad signatures, expired tokens, unexpected algorithms and invalid proofs cannot save", async () => {
  ...
  while (
    createHash("sha256")
      .update(challenge + badProof)
      .digest("hex")
      .startsWith("0000")
  )
    badProof = String(Number(badProof) + 1);
  assert.equal(
    (await POST(request(submission({ proof: badProof })))).status,
    403,
  );
  assert.equal(fakeStore.eval.mock.callCount(), 0);
});
...
test("challenge responses are fresh and not cacheable", async () => {
  ...
  assert.notEqual(
    (await first.json()).challenge,
    (await second.json()).challenge,
  );
});
...
test("save response waits for Redis; duplicates and failures are reported", async () => {
  ...
  let resolved = false;
  const response = POST(request(submission())).then((result) => {
    resolved = true;
    return result;
  });
  await started;
  ...
  for (const result of ["challenge_used", "ip_used"]) {
    ...
    assert.equal(
      (
        await createApi(new DB()).POST(request(submission()), {
          ip: "192.0.2.1",
        })
      ).status,
      403,
  ...
  try {
    fakeStore.eval.mock.mockImplementation(async () => {
...
test("sequential and concurrent proof replays reach Redis only once", async () => {
  const results = await Promise.all(
    Array.from({ length: 20 }, () => POST(request(submission()))),
  );
  assert.equal(results.filter((r) => r.status === 200).length, 1);
  assert.equal(results.filter((r) => r.status === 403).length, 19);
  assert.equal(fakeStore.eval.mock.callCount(), 1);
  assert.equal((await POST(request(submission()))).status, 403);
  assert.equal(fakeStore.eval.mock.callCount(), 1);
});

test("an invalid proof cannot consume another visitor's challenge", async () => {
  let invalid = "0";
  while (
    createHash("sha256")
      .update(challenge + invalid)
      .digest("hex")
      .startsWith("0000")
  )
    invalid = String(Number(invalid) + 1);
  assert.equal(
    (await POST(request(submission({ proof: invalid })))).status,
    403,
  );
...
test("sampling handles empty data, few distinct heights and huge counts", async () => {
  ...
  assert.deepEqual(
    await sampleDB({ 800: 2, 900: 1 }).getFoldSample(),
    [800, 800, 900],
  );
  assert.deepEqual(await sampleDB({ 800: 1000 }).getFoldSample(), [800]);
  assert.deepEqual(
    await sampleDB({ 800: Number.MAX_SAFE_INTEGER }).getFoldSample(),
    [800],
  );
  const data = Object.fromEntries(
    Array.from({ length: 1500 }, (_, i) => [String(i + 1), 2]),
  );
...
if (process.env.REQUIRE_REDIS_TESTS === "1" && !hasRedis)
  throw new Error("CI requires redis-server for integration tests");
test(
  "real Redis atomically deduplicates concurrent writes across DB instances",
  ...
  async (t) => {
    ...
    const results = await Promise.all(
      Array.from({ length: 20 }, () =>
        new DB(store).addFold(800, "192.0.2.1", "one-challenge"),
      ),
    );
    assert.equal(results.filter((result) => result === "saved").length, 1);
    assert.equal(await client.hget("folds", "800"), "1");
    assert.equal(
      await new DB(store).addFold(800, "192.0.2.2", "one-challenge"),
      "challenge_used",
    );
    assert.equal(
      await new DB(store).addFold(800, "192.0.2.1", "another-challenge"),
      "ip_used",
    );
    assert.equal(await client.exists("challenge:another-challenge"), 0);
    const ipRace = await Promise.all(
      Array.from({ length: 20 }, (_, i) =>
        new DB(store).addFold(900, "192.0.2.3", `unique-${i}`),
      ),
    );
    ...
    const challengeRace = await Promise.all(
      Array.from({ length: 20 }, (_, i) =>
        new DB(store).addFold(1000, `198.51.100.${i + 1}`, "shared-challenge"),
      ),
    );
    assert.equal(
      challengeRace.filter((result) => result === "saved").length,
      1,
    );
    assert.equal(await client.hget("folds", "1000"), "1");
    await client.hset("folds", "1100", "corrupt-count");
    await assert.rejects(
      new DB(store).addFold(1100, "203.0.113.1", "failed-write"),
    );
    assert.equal(
      await client.exists("challenge:failed-write", "ip:203.0.113.1"),
      0,
    );
    assert.ok((await client.ttl("challenge:one-challenge")) > 0);
    assert.ok((await client.ttl("challenge:one-challenge")) <= 120);
    const { visitorKey } = await import("../util-server");
    assert.ok((await client.ttl(visitorKey("192.0.2.1"))) > 120);
    assert.ok((await client.ttl(visitorKey("192.0.2.1"))) <= 1209600);
    assert.equal(await client.exists("ip:192.0.2.1"), 0);
    await client.set("ip:192.0.2.99", "1", "EX", 1000);
    assert.equal(
      await new DB(store).addFold(800, "192.0.2.99", "legacy"),
      "ip_used",
    );
    assert.equal(await client.exists(visitorKey("192.0.2.99")), 0);
    await client.set("ip:198.51.100.200", "1", "EX", 1000);
    const { restoreBackup } = await import("../scripts/backup");
    await restoreBackup(store as any, {
      version: "2.0.0",
      timestamp: new Date().toISOString(),
      data: { folds: { 700: 8, 900: 12 } },
    });
    assert.deepEqual(await client.hgetall("folds"), { 700: "8", 900: "12" });
    assert.equal(await client.ttl("folds"), -1);
    assert.ok((await client.ttl("ip:198.51.100.200")) > 0);
    await assert.rejects(
      restoreBackup(store as any, {
        version: "2.0.0",
        timestamp: new Date().toISOString(),
        data: { folds: { 900: -1 } },
      }),
    );
    assert.equal(await client.hget("folds", "900"), "12");
  },
);
